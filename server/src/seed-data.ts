...
`)
  ...
  for (const u of users) {
    insertUser.run(u.email, bcrypt.hashSync(u.password, SEED_BCRYPT_ROUNDS), u.name, u.role)
  }
  ...
  return { users: users.length, customers: customers.length, feedback: 80, notes: notes.length }
}
